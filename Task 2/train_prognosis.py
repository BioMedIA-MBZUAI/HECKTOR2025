...
def main():
    ...
    # Configuration
    IMAGE_DIR = "/l/users/salma.hassan/Task_2_Resampled"
    FEATURE_DIM = 64  # ResNet feature dimension
    EPOCHS = 50
    BATCH_SIZE = 2  # Small batch size for medical images
    LEARNING_RATE = 1e-4
    CHECKPOINT_DIR = "/l/users/salma.hassan/Prognosis/checkpoints"
    ...
    # Load data
    df = pd.read_csv('/l/users/salma.hassan/HECKTOR_2025_Training_Task_2.csv')
    
    # Load splits
    with open('/l/users/salma.hassan/Prognosis/cv_folds_5fold.json', 'r') as f:
        splits = json.load(f)
